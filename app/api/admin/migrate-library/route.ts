...
// One-time migration endpoint: seed `library_entries` from the spreadsheet's
// "Library" tab. Office-only. Safe to call multiple times only if the table
// is empty first (it does not de-duplicate).
export async function POST() {
  ...
  return NextResponse.json({ rowsRead: rows.length, inserted: entries.length });
}
